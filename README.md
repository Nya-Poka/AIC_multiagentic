...
Experiment Partner 输出结构化研究协议，而不是“研究条件需具体化”一类占位模板。协议包含
设计类型、因果解释边界、目标人群、纳入/排除标准、可测量变量、干预/暴露、对照、研究周期、
样本量与功效分析、分配与盲法、混杂因素、数据采集、统计分析、缺失数据、复现配置及伦理要求。
配置统一 LLM 后默认使用模型生成，并以严格 schema 校验；模型不可用或输出不合格时自动回退
本地确定性方案。可显式关闭模型实验规划：

```powershell
$env:RESEARCH_MESH_EXPERIMENT_USE_LLM = 'false'
```

Review Partner 会硬性拒绝以下结果：缺少必需协议字段、仍含“待定/需具体化”等占位内容、变量
没有量表/单位/设备/评分等操作化定义、样本量方案没有功效与效应量依据、统计方案没有模型、
效应估计和敏感性分析，或缺少复现及伦理审批要求。被拒绝的闭环不会伪装成 completed。

### 文献语义去重与来源质量分级

文献候选先按 DOI 合并，再按 Unicode/标点标准化标题、发表年份、第一作者和高阈值标题相似度
识别同一研究的不同数据库记录或会议/期刊版本。合并结果保留其他 DOI、标题、来源与匹配方法，
报告同时给出检索记录数、唯一候选数和移除的重复数，避免静默丢失溯源信息。

每条外部记录还会获得主题直接性、来源质量分数、A-D 级和文献类型。评分依据仅限可核验的
元数据完整性（稳定标识、摘要、作者、年份、载体、多源一致性和开放获取位置），不把引用次数
等同于研究结论正确。以其他暴露为标题主轴、仅在摘要中顺带提及目标关系的间接文献，以及
低于来源质量阈值的记录会被过滤并进入 `rejected_records` 审计列表。阈值可通过以下变量调整：

```powershell
$env:RESEARCH_MESH_LITERATURE_MIN_DIRECTNESS = '0.55'
